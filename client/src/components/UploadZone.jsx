...
export default function PublicVerifyPage() {
  ...
  const getErrorMessage = (error, defaultMessage) => {
    if (error.response?.status === 404) {
      return "Document not found on blockchain";
    }
    if (error.response?.status === 410) {
      return "Document has been revoked and is no longer valid";
    }
    return error.response?.data?.error || defaultMessage;
  };

  const fetchDocumentDetails = async (hash) => {
    const response = await axios.get(
      `http://localhost:5000/api/documents/verify/${hash.trim()}`
    );
  ...
  const handleVerifyByHash = async () => {
    if (!hashInput.trim()) {
      setResult({
        isValid: false,
        message: "Please enter a document hash",
      });
      return;
    }
    ...
    try {
      const document = await fetchDocumentDetails(hashInput);
      setResult({
        isValid: true,
      ...
      setResult({
        ...
        message: getErrorMessage(
          error,
          "Verification failed. Please check the hash and try again."
        ),
      });
    } finally {
      setVerifying(false);
  ...
  const handleVerifyByFile = async () => {
    ...
    try {
      ...
      if (verified) {
        const document = await fetchDocumentDetails(hash);
        setResult({
          isValid: true,
          message: "Document is verified and active on blockchain",
          document,
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-purple-200">
            Check if your document is authentic and registered on blockchain
          </p>
        </div>
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-3xl shadow-2xl p-8 border border-white/20">
          ...
          <button
            ...
            disabled={
              verifying ||
              (verificationMethod === "hash" ? !hashInput.trim() : !file)
            }
            ...
            >
              ...
              <p
                className={`mb-6 text-lg ${
                  result.isValid ? "text-green-200" : "text-red-200"
                }`}
              >
                {result.message}
              </p>
                      ...
                      <div>
                        ...
                        <p
                          className={`font-semibold ${
                            result.document.expiryDate === "Lifetime"
                              ? "text-green-400"
                              : "text-white"
                          }`}
                        >
          ...
          <div className="mt-8 p-4 bg-white/5 rounded-xl border border-white/10">
            <p className="text-sm text-purple-200 text-center">
              ...
              All verifications are performed against the immutable blockchain
              ledger
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-purple-300 text-sm">
                  Enter the unique hash code found on your document or
                  certificate
                </p>
              </div>
            </div>
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-purple-300 text-sm">
                  Upload the original PDF document to verify its authenticity
                </p>
              </div>
            </div>
          </div>
